...
$prof=new plugin_addressing_Profile();
...
if (isset($_POST["add"])){
	...
	foreach ($_POST["item"] as $key => $val){
		if ($val==1) {
			...
			$query="DELETE FROM glpi_plugin_addressing_profiles WHERE ID='".$key."'";
			$DB->query($query);
		}
	}		
...
echo "<div align='center'><form method='post' name='massiveaction_form' id='massiveaction_form'  action=\"./plugin_addressing.profile.php\">";

echo "<table class='tab_cadre' cellpadding='5'><tr><th colspan='4'>";
echo $LANGADDRESSING["profile"][4]." : </th></tr>";
echo "<tr><th></th><th></th><th>".$LANG["Menu"][35]."</th><th>".$LANGADDRESSING["profile"][3]."</th></tr>";

$query0="SELECT * FROM glpi_plugin_addressing_profiles ORDER BY name";
...
while ($data0=$DB->fetch_assoc($result0)){
	...
	if ($data0['addressing']=='r')
		echo "<td>".$LANG["profiles"][10]."</td>";
	elseif ($data0['addressing']=='w')
...
echo "<div align='center'><form method='post' action=\"".$CFG_GLPI["root_doc"]."/plugins/addressing/front/plugin_addressing.profile.php\">";
