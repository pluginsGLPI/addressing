...
function plugin_addressing_display(&$result, $PluginAddressing) {
	...
	$network=$PluginAddressing->fields["network"];
	$ping=$PluginAddressing->fields["ping"];
	...
	foreach ($result as $num => $lines) {
		...
		if (count($lines)) {
			if (count($lines)>1) {
				$disp = $PluginAddressing->fields["ip_double"];
			} else {
				$disp = $PluginAddressing->fields["ip_alloted"];
			}
			if ($disp) foreach ($lines as $line){
				...
				if ($line["device_type"] != NETWORKING_TYPE){
					if (haveTypeRight($line["device_type"], "r")) {
						$output_iddev = "<a href='".$CFG_GLPI["root_doc"]."/".$INFOFORM_PAGES[$line["device_type"]]."?ID=".$line["on_device"]."'>".$name
							.(empty($name) || $_SESSION["glpiview_ID"]?" (".$line["on_device"].")":"")."</a>";
					} else {
						$output_iddev = $name.(empty($name) || $_SESSION["glpiview_ID"]?" (".$line["on_device"].")":"");
					}
				}else{
					if (haveTypeRight($line["device_type"], "r")) {
						$output_iddev = "<a href='".$CFG_GLPI["root_doc"]."/".$INFOFORM_PAGES[$line["device_type"]]."?ID=".$line["on_device"]."'>".$namep." - ".$name
							.(empty($name) || $_SESSION["glpiview_ID"]?" (".$line["on_device"].")":"")."</a>";
					} else {
						$output_iddev = $namep." - ".$name.(empty($name) || $_SESSION["glpiview_ID"]?" (".$line["on_device"].")":"");
					}
				}
				...
				// Reserved
				if ($PluginAddressing->fields["ip_reserved"] && strstr($line["pname"],"reserv")) {
		...
		} else if ($PluginAddressing->fields["ip_free"]) {
			$row_num++;
			$item_num=1;
			if (!$ping) {
