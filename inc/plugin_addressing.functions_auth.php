...
function plugin_addressing_initSession() {
	global $DB;
	
	if(TableExists("glpi_plugin_addressing")){
		if(FieldExists("glpi_plugin_addressing","ipdeb")){
			$profile=new plugin_addressing_Profile();
			...
			$_SESSION['glpi_plugin_addressing_profile'] = array ();
			if ($DB->numrows($result)) {
				while ($data = $DB->fetch_assoc($result)) {
					$profile->fields = array ();
					if(isset($_SESSION["glpiactiveprofile"]["ID"])){
						$profile->getFromDB($_SESSION["glpiactiveprofile"]["ID"]);
						$_SESSION['glpi_plugin_addressing_profile'] = $profile->fields;
					}else{
						$profile->getFromDB($data['ID']);
						$_SESSION['glpi_plugin_addressing_profile'] = $profile->fields;
					}
					$_SESSION["glpi_plugin_addressing_installed"]=1;
				}
			}
		}
	}
...
function plugin_addressing_checkRight($module, $right) {
	global $CFG_GLPI;
